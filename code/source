package PowerSysPro
  package Components "Components for load flow calculations"
    model mySource "Perfect voltage source"
      ...
      Interfaces.myTerminal terminal(v(re(start=UNom/sqrt(3), nominal=UNom/sqrt(3)), im(start=0))) "Terminal of the node" annotation (Placement(
          visible=true,
          transformation(
            origin={0,0},
            extent={{-10,-10},{10,10}},
    ...
    equation
      ...
      annotation (
        Icon(coordinateSystem(grid = {0.1, 0.1}, initialScale = 0.1), graphics={  Text(origin = {60.1, -63.1}, lineColor = {0, 0, 255}, extent = {{-124, 11}, {124, -11}}, textString = if OnePhaseLoad then "1" else "3", textStyle = {TextStyle.Bold}),
            coordinateSystem(preserveAspectRatio = false), Text(extent={{-40.5,-6},{41.5,-46}}, lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~")}),
        Documentation(info = "<html>
    ...
    model myVariablePVNode "variable PV node for islanding"
      ...
      Modelica.Blocks.Interfaces.RealInput Qreg annotation (
        Placement(transformation(extent={{-85.8,47.2},{-63.4,69.6}}),
                                                                   iconTransformation(extent={{-85.8,
                47.2},{-63.4,69.6}})));
    ...
    equation
      ...
      annotation (
        Icon(coordinateSystem(grid = {0.1, 0.1}, initialScale = 0.1), graphics={  Text(origin = {60.1, -63.1}, lineColor = {0, 0, 255}, extent = {{-124, 11}, {124, -11}}, textString = if OnePhaseLoad then "1" else "3", textStyle = {TextStyle.Bold}),
                                                                                  Text(extent={{
                  -40.5,-5.7},{41.5,-45.7}},                                                                                    lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~")}),
        Documentation(info = "<html>
    <p>This node prescribes the variable active power <code>P</code> entering the PV bus.</p>
    </html>"));
    end myVariablePVNode;
    ...
    model myLine "MV or LV line with constant impedance"
      ...
      Interfaces.myTerminal terminalA(i(re(start=0), im(start=0)))  "Terminal A of the node" annotation (Placement(
          visible=true,
          transformation(
            origin={-100,0},
            extent={{-10,-10},{10,10}},
          ...
          iconTransformation(
            ...
            extent={{-10,-10},{10,10}},
            rotation=0)));
      Interfaces.myTerminal terminalB(v(re(start=UNom/sqrt(3), nominal=UNom/sqrt(3)), im(start=0))) "Terminal B of the node" annotation (Placement(
          visible=true,
          transformation(
            origin={100,0},
            extent={{-10,-10},{10,10}},
    ...
    model myLineWithFault "MV or LV line with constant impedance and fault at intermediate position"
      ...
      PartialModels.myFault myFault(R = RFault, X = XFault, startTime = startTime, stopTime = stopTime) annotation (
        Placement(visible = true, transformation(origin={0,0},    extent={{-22,-22},
                {22,22}},                                                                          rotation = 0)));
    public
      parameter Types.myVoltage UNom "Reference voltage of the line";
      parameter Types.myResistance R "Series resistance of phase conductor";
    ...
    equation
      ...
      connect(terminalA, terminalA)   annotation (Line(points={{-100,0},{-100,0}}, color={0,0,0}));
      annotation (
        Documentation(info = "<html>
    <p>A line can be modeled as a dedicated Pi-network. A diagram of the model is shown in the next figure:</p>
    <p>  </p>
      ...
      partial model myPartialTransformer "Partial model for transformers"
        ...
        Interfaces.myTerminal terminalA(i(re(start = 0), im(start = 0))) "Terminal A of the node" annotation (Placement(
            ...
            iconTransformation(
              ...
              extent={{-10,-10},{10,10}},
              rotation=0)));
        Interfaces.myTerminal terminalB(v(re(start = UNomB / sqrt(3), nominal = UNomB / sqrt(3)), im(start = 0))) "Terminal B of the node" annotation (Placement(
            visible=true,
            transformation(
              origin={100,0},
              extent={{-10,-10},{10,10}},
    ...
    <p>The model describes a transformer with fixed transformer ratio with Ya=0, Yb=G+jB and k = UNomB/UNomA*exp(j*theta).</p>
      ...
      model myFault "Modeling a fault during a time interval"
        Interfaces.myTerminal terminal "Port terminal" annotation (Placement(
            visible=true,
            transformation(
  ...
  package Buses "Bus for causal connections"
    ...
    model myCausalBus_in "Causal input bus"
      extends Icons.myBus;
      Interfaces.myTerminal terminal annotation (Placement(
          visible=true,
          transformation(
    ...
    <p> Causal input connector, with complex voltage as input and complex current as output.</p>    
        ...
        Icon(graphics={  Text(origin = {73, 97}, lineColor = {0, 0, 255}, extent = {{-131, 11}, {131, -11}}, textString = "%name"), Polygon(origin={-6,
                  -1},                                                                                                                                         fillColor = {70, 70, 70}, pattern = LinePattern.None,
    ...
    <p> Causal output connector, with complex voltage as output and complex current as input.</p>
        ...
        Icon(graphics={  Text(origin = {-79, 97}, lineColor = {0, 0, 255}, extent = {{-131, 11}, {131, -11}}, textString = "%name"), Polygon(origin = {8, -1}, fillColor = {70, 70, 70}, pattern = LinePattern.None,
                fillPattern =                                                                                                                                                                                                      FillPattern.Solid, points = {{0, 19}, {40, 0}, {0, -21}, {0, 19}}), Text(extent={{
                  -70,-10},{12,-50}},                                                                                                                                                                                                        lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~")}));
    end myCausalBus_out;
    annotation (
...
The myTerminal connector represents an AC terminal with voltage and flow current.
  ...
  package Types "Domain-specific type definitions"
    ...
    operator record myComplexAdmittance = Modelica.SIunits.ComplexAdmittance;
    operator record myComplexImpedance = Modelica.SIunits.ComplexImpedance;
  ...
  package Icons "Icons for the library models"
    ...
    annotation (
        Icon(graphics={  Ellipse(origin = {0, -1}, extent = {{-60, 61}, {60, -59}}, endAngle = 360), Text(origin = {0, 96}, lineColor = {0, 0, 255}, extent = {{-118, 12}, {118, -12}}, textString = "%name"), Text(origin = {-138.43, -89.33}, lineColor = {28, 108, 200}, extent = {{-53.57, 7.33}, {331.14, -14.66}}, textStyle = {TextStyle.Bold}, textString = "%UNom"),
                         Line(origin={6.7702,12.33}, points = {{-51.4286, -11.4286}, {-44.9714, 8.11429}, {-40.8571, 18.9143}, {-37.2, 26.5143}, {-33.9429, 31.2}, {-30.7429, 33.7714}, {-27.5429, 34.1714}, {-24.3429, 32.3429}, {-21.0857, 28.4}, {-17.8857, 22.5143}, {-14.2286, 13.7714}, {-9.61714, 0.685714}, {0.0571429, -29.0286}, {4.17143, -40.1143}, {7.82857, -48.1143}, {11.0286, -53.2}, {14.2857, -56.2286}, {17.4857, -57.1429}, {20.6857, -55.7714}, {23.9429, -52.2857}, {27.1429, -46.8}, {30.8, -38.4}, {35.4286, -25.6}, {40, -11.4286}}, smooth = Smooth.Bezier)},
    ...
    model myTransformer "Icon for transformer"
      annotation (
        Icon(graphics={  Ellipse(origin = {-15, -7}, extent = {{-45, 47}, {35, -33}}, endAngle = 360), Ellipse(origin = {3, -9}, extent = {{57, 49}, {-23, -31}}, endAngle = 360), Line(origin = {-80, 0}, points = {{-20, 0}, {20, 0}, {20, 0}}), Line(origin = {80, 0}, points = {{-20, 0}, {20, 0}}), Text(origin = {-1, 94}, lineColor = {0, 0, 255}, extent = {{-179, 14}, {179, -14}}, textString = "%name"), Text(origin = {-91.9974, -90.6665}, lineColor=
                  {28,108,200},                                                                                                                                                                                                        extent = {{-92.004, 6.66649}, {281.997, -11.3335}}, textStyle=
                  {TextStyle.Bold},
              textString="%SNom")},                                                                                                                                                                                                      coordinateSystem(initialScale = 0.1)));
    end myTransformer;
    ...
    model myTwoPortsAC "Completing two ports AC icon"
      annotation (
        Icon(                                                                                                                                                                                                        coordinateSystem(initialScale = 0.1),
            graphics={   Text(extent={{-144,52},{-52,28}},      lineColor = {0, 0, 0}, textString = "A", textStyle = {TextStyle.Bold}), Text(extent={{
                  48,52},{160,28}},                                                                                                                                          lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "B"), Text(extent={{
                  -140,-12},{-58,-52}},                                                                                                                                                                                                        lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~"),
                                                                                                                                                                                                        Text(extent={{
                  60,-10},{142,-50}},                                                                                                                                                                                                        lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~")}));
    end myTwoPortsAC;
      ...
      annotation (
        Icon(                                                                                                                                                                                                        coordinateSystem(initialScale = 0.1),
            graphics={                                                                                                                                                                                                        Text(extent={{
                  -40,-12},{42,-52}},                                                                                                                                                                                                        lineColor = {0, 0, 0}, textStyle = {TextStyle.Bold}, textString = "~")}));
    end myOnePortAC;
...
<p>This icon shall be used for the contact information of the library developers.</p>
  ...
  package Tests "Tests package based on single component testing"
    ...
    model OneSource "Testing a source"
      ...
      flow Modelica.Blocks.Interfaces.RealInput i_re(start = -30) annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=180,
            origin={100,24})));
      flow Modelica.Blocks.Interfaces.RealInput i_im(start = -40) annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=180,
            origin={100,-8})));
    equation
      ...
      annotation (
        Diagram(graphics={  Text(lineColor = {28, 108, 200}, extent={{-108,24},
                  {34,14}},                                                                   fontSize = 12, textString = "U = 10 kV, theta = 30°"), Text(lineColor = {28, 108, 200}, extent = {{-94, -20}, {106, -60}}, fontSize = 12, textString = "current in the source is 50 A
apparent power flowing the source is 866 kVA"),
                            Text(lineColor={28,108,200},     extent={{10,48},{
                  152,38}},                                                                   fontSize=
                  12,
              textString="i_re = -30, i_im = -40")},
                                                 coordinateSystem(initialScale = 0.1)));
    end OneSource;
    ...
    model OneLoad "Testing a load"
      ...
      Modelica.Blocks.Interfaces.RealInput v_re(start = 4000/sqrt(3))   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=0,
            origin={-100,26})));
      Modelica.Blocks.Interfaces.RealInput v_im(start = 3000/sqrt(3))   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=0,
            origin={-100,-10})));
    equation
      connect(Bin.terminal, load.terminal) annotation (
        Line(points={{31,6},{48,6},{48,6.02},{65.96,6.02}},
                                            color = {0, 0, 0}));
      connect(v_re, Bin.v_re) annotation (Line(points={{-100,26},{-18,26},{-18,2},{26.2,
              2}}, color={0,0,127}));
      connect(Bin.v_im, v_im) annotation (Line(points={{26.2,-2},{-6,-2},{-6,-10},{-100,
              -10}}, color={0,0,127}));
      annotation (
...
apparent power flowing the load is 5 kVA"), Text(extent = {{10, 54}, {140, 12}}, lineColor = {28, 108, 200}, fontSize = 12, textString = "P = 4 kW, Q = 3 kvar"),
                            Text(lineColor={28,108,200},     extent={{-220,58},{48,52}},      fontSize=
                  12,
              textString="v_re = 4000/sqrt(3), v_im = 3000/sqrt(3)")},                                                                                                                                                                                                        coordinateSystem(initialScale = 0.1)));
    end OneLoad;
    ...
    equation
      ...
      connect(Bout.i_re, i_re) annotation (Line(points={{28,22},{58,22},{58,34},
              {104,34}}, color={0,0,127}));
      connect(Bout.i_im, i_im) annotation (Line(points={{28,18},{56,18},{56,-2},
              {104,-2}}, color={0,0,127}));
      connect(v_re, Bin.v_re) annotation (Line(points={{-110,40},{-56,40},{-56,10},{
              -37.8,10}},
                   color={0,0,127}));
      connect(v_im, Bin.v_im) annotation (Line(points={{-110,4},{-74,4},{-74,6},{-37.8,
              6}},           color={0,0,127}));
      annotation (
...
apparent power flowing the line is 433 kVA"),
                             Text(origin = {42.8, 22.3078}, lineColor = {28, 108, 200}, extent = {{-76.8, 19.6922}, {-12.8, 35.6922}}, fontSize = 12, textString = "perfect line:
R very low, and X=G=B=0"),  Text(lineColor={28,108,200},     extent={{30,60},{
                  172,50}},                                                                   fontSize=
                  12,
              textString="i_re = -30, i_im = -40"),
                            Text(lineColor={28,108,200},     extent={{-208,74},{20,66}},      fontSize=
                  12,
              textString="v_re = 4000/sqrt(3), v_im = 3000/sqrt(3)")},                                                                                                                                                                            coordinateSystem(initialScale = 0.1)));
    end OnePerfectLine;
    ...
    model OneLine "Testing a normal line"
      ...
      flow Modelica.Blocks.Interfaces.RealInput i_im(start = -40)   annotation (Placement(transformation(
            ...
            rotation=180,
            origin={100,4})));
      Modelica.Blocks.Interfaces.RealInput v_re(start = 5000/sqrt(3))   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=0,
            origin={-112,34})));
      Modelica.Blocks.Interfaces.RealInput v_im(start = 0)   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=0,
            origin={-112,-2})));
    equation
      ...
      connect(Bout.i_re, i_re) annotation (Line(points={{30,20},{52,20},{52,40},{100,
              40}}, color={0,0,127}));
      connect(Bout.i_im, i_im) annotation (Line(points={{30,16},{54,16},{54,4},{100,
              4}}, color={0,0,127}));
      connect(v_im, Bin.v_im) annotation (Line(points={{-112,-2},{-74,-2},{-74,4},{-37.8,
              4}}, color={0,0,127}));
      connect(v_re, Bin.v_re) annotation (Line(points={{-112,34},{-76,34},{-76,8},{-37.8,
              8}}, color={0,0,127}));
      annotation (
...
R = 10 ohms, and X=G=B=0"), Text(extent = {{-58, -18}, {70, -68}}, lineColor={
                  28,108,200},                                                                 fontSize=
                  12,
              textString="current in the line is 50 A and voltage is 5 kV 
voltage drop is 466 V
apparent power flowing the line is 433 kVA"),
                            Text(lineColor={28,108,200},     extent={{26,66},{168,56}},       fontSize=
                  12,
              textString="i_re = -30, i_im = -40"),
                            Text(lineColor={28,108,200},     extent={{-188,64},{-46,
                  54}},                                                                       fontSize=
                  12,
              textString="v_re = 5000/sqrt(3), v_im = 0")},
                                               coordinateSystem(initialScale = 0.1)));
    end OneLine;
    ...
    model OneperfectTransfo "Testing a perfect transformer"
      ...
      flow Modelica.Blocks.Interfaces.RealInput i_im(start = -40)   annotation (Placement(transformation(
            ...
            rotation=180,
            origin={100,6})));
      Modelica.Blocks.Interfaces.RealInput v_re(start = 63000/sqrt(3))   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=0,
            origin={-106,40})));
      Modelica.Blocks.Interfaces.RealInput v_im(start = 0)   annotation (Placement(transformation(
    ...
    equation
      ...
      connect(Bout.i_re, i_re) annotation (Line(points={{28,24},{54,24},{54,42},
              {100,42}}, color={0,0,127}));
      connect(Bout.i_im, i_im) annotation (Line(points={{28,20},{52,20},{52,6},
              {100,6}}, color={0,0,127}));
      connect(v_re, Bin.v_re) annotation (Line(points={{-106,40},{-84,40},{-84,
              12},{-37.8,12}},
                   color={0,0,127}));
      connect(v_im, Bin.v_im) annotation (Line(points={{-106,4},{-70,4},{-70,8},
              {-37.8,8}}, color={0,0,127}));
      annotation (
...
ratio=20/63"),                                                                                                                                                                                                        Text(extent = {{-66, -14}, {62, -64}}, lineColor=
                  {28,108,200},                                                                                                                                                                                                        fontSize=
                  12,
              textString="voltage at port A is 63 kV and is 20 kV at port B
current in port A is 15.9 A and is 50 A in port B
apparent power flowing the transformer is 1732 kVA"),
                            Text(lineColor={28,108,200},     extent={{26,68},{
                  168,58}},                                                                   fontSize=
                  12,
              textString="i_re = -30, i_im = -40"),
                            Text(lineColor={28,108,200},     extent={{-200,66},
                  {-6,62}},                                                                   fontSize=
                  12,
              textString="v_re = 63000/sqrt(3), v_im = 0")},
                                                       coordinateSystem(initialScale = 0.1)));
    ...
    model OneTransfo "Testing a normal transformer"
      ...
      Buses.myCausalBus_out Bout annotation (
        Placement(transformation(extent = {{16, 6}, {36, 26}})));
      flow Modelica.Blocks.Interfaces.RealInput i_re(start = -50)   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=180,
            origin={100,50})));
      flow Modelica.Blocks.Interfaces.RealInput i_im(start = 0)   annotation (Placement(transformation(
            extent={{-20,-20},{20,20}},
            rotation=180,
            origin={100,14})));
      Modelica.Blocks.Interfaces.RealInput v_re(start = 63000/sqrt(3))   annotation (Placement(transformation(
    ...
    equation
      ...
      connect(Bout.terminal, tra.terminalB) annotation (
        Line(points={{23,16},{6,16}},        color = {0, 0, 0}));
      connect(Bout.i_re, i_re) annotation (Line(points={{28,24},{54,24},{54,50},{100,
              50}}, color={0,0,127}));
      connect(Bout.i_im, i_im) annotation (Line(points={{28,20},{52,20},{52,14},{100,
              14}}, color={0,0,127}));
      connect(v_re, Bin.v_re) annotation (Line(points={{-104,44},{-70,44},{-70,
              12},{-37.8,12}},
                   color={0,0,127}));
      connect(v_im, Bin.v_im)
        annotation (Line(points={{-104,8},{-37.8,8}}, color={0,0,127}));
      annotation (
...
ratio=20/63"),                                                                                                                                                                                                        Text(extent = {{-132, -8}, {142, -58}}, lineColor=
                  {28,108,200},                                                                                                                                                                                                        fontSize=
                  12,
              textString="voltage at port A is 63 kV
voltage drop at port B is 87 V bellow 20 kV
current in port A is 15.9 A and is 50 A in port B
apparent power flowing the transformer is 1732 kVA"),
                            Text(lineColor={28,108,200},     extent={{24,76},{166,66}},       fontSize=
                  12,
              textString="i_re = -50, i_im = 0"),
                            Text(lineColor={28,108,200},     extent={{-198,70},
                  {-4,66}},                                                                   fontSize=
                  12,
              textString="v_re = 63000/sqrt(3), v_im = 0")},
                                                       coordinateSystem(initialScale = 0.1)));
    ...
    equation
      ...
      connect(v_re, Bin.v_re) annotation (Line(points={{-90,44},{-30,44},{-30,8},
              {28.2,8}},
                   color={0,0,127}));
      connect(v_im, Bin.v_im) annotation (Line(points={{-90,8},{-32,8},{-32,4},
              {28.2,4}}, color={0,0,127}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio = false)),
...
current is 2887 A
apparent power flowing the bank is 25 MVAR"),
                            Text(lineColor={28,108,200},     extent={{-242,74},
                  {62,66}},                                                                   fontSize=
                  12,
              textString="v_re = 4000/sqrt(3), v_im = 3000/sqrt(3)")}));
    end OneBank;
...
90 s, 100 s, 110 s, 120 s, and 130 s"), Text(lineColor = {28, 108, 200}, extent = {{-132, 58}, {230, 28}}, fontSize = 12,
    ...
    model QfURegulation "Testing the regulation Q=f(U)"
      ...
      Modelica.Blocks.Sources.Sine sine(amplitude = 300, freqHz = 0.01, offset = 1000) annotation (
        Placement(transformation(extent = {{-66, 28}, {-32, 62}})));
    equation
      connect(sine.y, qfU.U) annotation (
        Line(points = {{-30.3, 45}, {-8, 45}, {-8, 46.4}, {5.9, 46.4}}, color = {0, 0, 127}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio = false)),
  ...
  package Examples "Some basic examples"
    ...
    equation
      connect(src.terminal, load.terminal) annotation (
        Line(points={{-26,18},{4,18},{4,18.02},{35.96,18.02}},
                                             color = {0, 0, 0}));
      annotation (
...
apparent power flowing the components is 5 kVA")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceOneLineOneLoad
      ...
      Components.myLine line(
        UNom=10000,          Imax = 120, R = 20, X = 0) annotation (
...
voltage drop is 10 V in the resistive line")}, coordinateSystem(initialScale = 0.1)));
    ...
    model TwoSourcesTwoLinesOneLoad
      ...
      Components.myLine line1(
        UNom=10000,           Imax = 50, R = 1) annotation (
    ...
    equation
      ...
      annotation (
        Diagram(graphics={  Text(lineColor={28,108,200},     extent = {{-38, -20}, {36, -64}}, fontSize=
                  12,
              textString="current is 0.289 A in the load
and equally divided in the two lines"), Text(lineColor = {28, 108, 200}, extent = {{4, 36}, {174, 12}}, fontSize = 12, textString = "P=4 kW and Q=3 kvar")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneProdLoadOneLineOneLineOneLoad
      extends Modelica.Icons.Example;
      Components.myPVNode pv(UNom = 400, Pstart = -5000) annotation (
      ...
      Components.myLine line1(
        UNom=400,             Imax = 50, R = 0.1) annotation (
        Placement(visible = true, transformation(origin = {-30, 14}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line2(
        UNom=400,             Imax = 50, R = 0.11) annotation (
        Placement(visible = true, transformation(origin = {-30, -10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
    equation
      connect(pv.terminal, line1.terminalB) annotation (
        Line(points = {{10, 14}, {-20, 14}}, color = {0, 0, 0}));
      connect(line2.terminalB, load.terminal) annotation (
        Line(points={{-20,-10},{-8,-10},{-8,-9.98},{3.96,-9.98}},
                                              color = {0, 0, 0}));
      connect(line1.terminalA, line2.terminalA) annotation (
        Line(points = {{-40, 14}, {-66, 14}, {-66, -10}, {-40, -10}}, color = {0, 0, 0}));
      annotation (
        Diagram(graphics={  Text(lineColor = {28, 108, 200}, extent = {{-6, 54}, {70, 32}}, fontSize = 12, textString = "P=5 kW and Q=0.1 kvar for the load
Pstart=-5 kW for the PV node"), Text(lineColor = {28, 108, 200}, extent = {{-32, -22}, {42, -66}}, fontSize = 12, textString = "the load is correctly supplied by the PV node
and the voltage is correct (397 V)")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceOneTransfo
      ...
      Components.myTransformer tra(
        UNomA=10000,
        UNomB=5000, R = 0.3, SNom = 0, X = 12.4) annotation (
...
current is null along the feeder")}));
    ...
    model OneSourceOneTransfoOneLineOneTransfoThreeLines
      ...
      Components.myLine line1(
        UNom=20000,           Imax = 20, R = 125e-4, X = 125e-4) annotation (
        Placement(visible = true, transformation(origin={0,20},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myTransformer tra2(
        UNomA=20000,
        UNomB=400,
        SNom=0.1, R = 0.2, X = 21.22) annotation (
        Placement(visible = true, transformation(origin={32,20},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line2(
        UNom=400,             Imax = 120, R = 2.157870e-02, X = 2.657870e-02) annotation (
...
current is null along the feeders")}));
    ...
    equation
      ...
      annotation (
        Diagram(graphics={  Text(lineColor={28,108,200},     extent={{-34,-22},
                  {40,-66}},                                                                   fontSize=
                  12,
              textString="upstream voltage is 20 kV
downstream voltage is 10 kV
current in load is 0.289 A
current in source is 0.144 A
voltage drop at the port B of the transformer is 1.25 V"),
                                        Text(lineColor = {28, 108, 200}, extent = {{-14, 52}, {144, 32}}, fontSize = 12, textString = "P=5 kW and Q=0 kvar")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceThreeLoads
      ...
      Components.myLine line1(
        UNom=10000,           Imax = 50, R = 10) annotation (
    ...
    equation
      ...
      connect(line3.terminalB, load3.terminal) annotation (
        Line(points={{4,4},{12,4},{12,4.02},{21.96,4.02}},
                                         color = {0, 0, 0}));
      connect(line2.terminalB, load2.terminal) annotation (
        Line(points={{4,26},{12,26},{12,26.02},{21.96,26.02}},
                                           color = {0, 0, 0}));
...
current in the source 0.867 A")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceOneTransfoOnelineOneLoad
      extends Modelica.Icons.Example;
      Components.myLine line(
        UNom=10000,          Imax = 50, R = 10, X = 2.657870e-02) annotation (
      ...
      Components.myTransformer tra(
        UNomA=20000,
        UNomB=10000,
        SNom=100, R = 0.1, X = 12.4) annotation (
...
current in the source is 0.144 A")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceTwoLinesOneLoad
      ...
      Components.myLine line1(
        UNom=10000,           R = 10, X = 2.657870e-02, Imax = 80) annotation (
    ...
    equation
      ...
      connect(line2.terminalB, load.terminal) annotation (
        Line(points={{30,10},{40,10},{40,10.02},{49.96,10.02}},
                                            color = {0, 0, 0}));
      annotation (
...
current is 0.289 A in the feeder")}, coordinateSystem(initialScale = 0.1)),
    ...
    model OneSourceOneLineOneTransfoOneLineOneLoad
      extends Modelica.Icons.Example;
      Components.myLine line2(
        UNom=400,             X = 2e-3, Imax = 60, R = 2e-3) annotation (
      ...
      Components.myTransformer tra1(
        UNomA=63000,
        UNomB=20000,
        SNom=100, R = 1e-9, X = 1e-9) annotation (
        Placement(visible = true, transformation(origin = {-46, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line1(
        UNom=20000,           Imax = 60, R = 0.4, X = 2.5e-02) annotation (
...
but voltage is correct")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceTwoTransfosOneLoad
      ...
      Components.myTransformer tra2(
        UNomA=20000,
        UNomB=400,
        SNom=0.25, R = 1e-9, X = 1e-9) annotation (
        Placement(visible = true, transformation(origin = {20, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      ...
      Components.myTransformer tra1(
        UNomA=63000,
        UNomB=20000,
        SNom=36, R = 1e-9, X = 1e-9) annotation (
    ...
    equation
      ...
      connect(tra2.terminalB, load.terminal) annotation (
        Line(points={{30,22},{56,22},{56,22.02},{51.96,22.02}},
                                            color = {0, 0, 0}));
...
and 0.046 A in the source")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceOneTransfoOneLineOneTransfoOneLineOneLoad
      ...
      Components.myTransformer tra2(
        UNomA=20000,
        UNomB=400,
        SNom=0.25,  R = 1e-6, X = 1e-6) annotation (
        Placement(visible = true, transformation(origin = {20, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line2(
        UNom=400,             Imax = 60, R = 1, X = 2.657870e-02) annotation (
        Placement(visible = true, transformation(origin = {52, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLoad load(UNom = 400, P = 5000) annotation (
        Placement(visible = true, transformation(origin={78,22},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line1(
        UNom=20000,           Imax = 60, R = 1, X = 125e-4) annotation (
        Placement(visible = true, transformation(origin = {-14, 22}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myTransformer tra1(
        UNomA=63000,
        UNomB=20000,
        SNom=70,  R = 0.2, X = 1e-6) annotation (
        Placement(visible = true, transformation(origin={-52,22},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.mySource src(UNom = 63000) annotation (
...
and 0.047 A in the source")}, coordinateSystem(initialScale = 0.1)));
    ...
    equation
      ...
      connect(bin2.terminal, load.terminal) annotation (
        Line(points={{85,20},{92,20},{92,20.02},{99.96,20.02}},
                                               color = {0, 0, 0}));
...
and 0.047 A in the source")}, coordinateSystem(initialScale = 0.1)));
    ...
    model OneSourceOneLineOneLoadOneBank
      ...
      Components.myLine line(
        UNom=20000,          Imax = 150, R = 2, X = 2) annotation (
...
and Imax is only 50 A")}));
    ...
    model OneSourceThreeLineOneSource
      extends Modelica.Icons.Example;
      ...
      Components.myLine line2(
        UNom=1000,            Imax = 60, R = 10, X = 1e-6) annotation (
        Placement(visible = true, transformation(origin = {0, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line3(
        UNom=1000,            Imax = 50, R = 10, X = 1e-6) annotation (
...
but Imax for line3 is only 50 A")}));
    end OneSourceThreeLineOneSource;

    model TwoSourcesOneLineSensors
      extends Modelica.Icons.Example;
      ...
      Components.mySource src1(UNom = 10000) annotation (
        Placement(visible = true, transformation(origin = {-60, 28}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myLine line(
        UNom=10000,          R = 100, X = 1e-6) annotation (
...
in voltage, current and apparent power in the line")}));
    end TwoSourcesOneLineSensors;

    model OneSourceOneLigneWithFault
      extends Modelica.Icons.Example;
      Components.myLineWithFault lnFault(
        UNom=10000,                      R = 1, Imax = 100, RFault = 0.3, X = 0, faultLocationPu = 0.7, startTime = 0.4, stopTime = 0.6) annotation (
...
<p>At time = 0.6 the fault is cleared and the current through the line returns to the original value.</p>
    ...
    model VariableLoad
      ...
      Components.myTransformer tra(
        UNomA=20000,
        UNomB=400,
        SNom=0.8, R = 0.1) annotation (
        Placement(transformation(extent={{2,64},{22,84}})));
      Components.myLine line2(
        UNom=400,             Imax = 25, R = 1) annotation (
        Placement(transformation(extent = {{38, 64}, {58, 84}})));
      Components.myLine line1(
        UNom=20000,           Imax = 10, R = 1) annotation (
    ...
    equation
      ...
      connect(varLoad.terminal, line2.terminalB) annotation (
        Line(points={{75.96,74.02},{66,74.02},{66,74},{58,74}},
                                            color = {0, 0, 0}));
...
simulation is done for 1-year duration")}),
        experiment(StopTime = 31532400, __Dymola_Algorithm = "Dassl"));
    end VariableLoad;
    ...
    model QfURegulation "Testing the regulation Q=f(U)"
      ...
      Components.myLine line(
        UNom=20000,          R = 1, X = 0, Imax = 60) annotation (
      ...
      Modelica.Blocks.Sources.Sine sine(amplitude = 30000000, freqHz = 0.01, offset = 0) annotation (
        Placement(transformation(extent = {{-17, -17}, {17, 17}}, rotation = 180, origin = {67, -31})));
      Components.myTransformer tra(
        UNomA=63000,
        UNomB=20000, R = 1,
        SNom=36000)  annotation (
    ...
    equation
      ...
      connect(sine.y, varLoad.PInput) annotation (
        Line(points = {{48.3, -31}, {35.46, -31}, {35.46, 27.55}}, color = {0, 0, 127}));
      ...
      connect(line.terminalB, sensor.terminal) annotation (Line(points={{-18,28},
              {-10,28},{-10,66},{-2,66}}, color={0,0,0}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio = false)),
        Diagram(coordinateSystem(preserveAspectRatio = false), graphics={  Text(lineColor = {28, 108, 200}, extent={{
                  -92,2},{32,-72}},                                                                                                          fontSize = 12, textString = "reactive power control done on
the MV line (injection and consumption)")}),
    ...
    package VariableTransformers
      ...
      model VariableTransformer1 "Comparing transformer with and without tap changer (variable voltage)"
        ...
        Components.myTransformer fixTra(
          UNomA=20000,
          UNomB=10000,
          SNom=100,  R = 1) annotation (
          Placement(visible = true, transformation(origin = {52, 62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        Components.myVariableTransformer varTra(
          UNomA=20000,
          UNomB=10000,
          SNom=100, R = 1) annotation (
...
and variable transformer with tap changer")}));
      ...
      model WithVariableLoad
        ...
        Components.myLine line1(UNom=63000,
                                R = 10) annotation (
          Placement(transformation(extent = {{-78, 22}, {-58, 42}})));
        Components.myVariableTransformer varTra(
          UNomA=63000,
          UNomB=20000,                          R = 1) annotation (
          Placement(transformation(extent = {{-48, 22}, {-28, 42}})));
        Components.myLine line2(UNom=20000,
                                R = 5) annotation (
      ...
      equation
        ...
        connect(line2.terminalB, varLoad.terminal) annotation (
          Line(points={{6,32},{12,32},{12,32.02},{19.96,32.02}},
                                             color = {0, 0, 0}));
...
the variable active power of the load")}),
    ...
    package BreakerTests
      ...
      model BreakersWithLines "Two lines with opposite breakers"
        ...
        Components.myLine line1(
          UNom=1000,            R = 10, X = 1e-6) annotation (
          Placement(transformation(extent = {{0, 56}, {20, 76}})));
        Components.myLine line2(
          UNom=100,             R = 10, X = 1e-6) annotation (
        ...
        Components.myGround ground annotation (Placement(transformation(
              extent={{-10,-10},{10,10}},
              rotation=0,
              origin={86,44})));
      equation
        ...
        connect(line1.terminalB, ground.terminal) annotation (Line(points={{20,
                66},{80,66},{80,44},{86,44}}, color={0,0,0}));
        connect(line2.terminalB, ground.terminal) annotation (Line(points={{22,
                18},{80,18},{80,44},{86,44}}, color={0,0,0}));
...
when a breaker is open a current is flowing into the ground")}));
      ...
      model Islanding "Two loads with opposite breakers"
        extends Modelica.Icons.Example;
        Components.mySource src(UNom = 20000) annotation (
        ...
        Components.myPVNode pv(UNom = 400, Pstart = -2000) annotation (
        ...
        Components.myLine line1(
          UNom=20000,           Imax = 50, R = 0.1) annotation (
          Placement(visible = true, transformation(origin={-62,30},    extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        Components.myTransformer tra(
          UNomA=20000,
          UNomB=400, R = 0.1) annotation (
          Placement(transformation(extent = {{-40, 20}, {-20, 40}})));
      equation
        connect(pv.terminal, line2.terminalB) annotation (
          Line(points = {{78, 42}, {54, 42}}, color = {0, 0, 0}));
        connect(line3.terminalB, load3.terminal) annotation (
          Line(points={{54,18},{62,18},{62,18.02},{71.96,18.02}},
                                              color = {0, 0, 0}));
        ...
        connect(brk.terminalB, line3.terminalA) annotation (
          Line(points = {{10, 30}, {22, 30}, {22, 18}, {34, 18}}, color = {0, 0, 0}));
...
Pmax=-2 kW for the PV node")}));
      ...
      model Islanding2 "Two loads with opposite breakers"
        ...
        Components.myBreaker brk1 annotation (
          Placement(transformation(extent = {{-10, 20}, {10, 40}})));
        Components.myLoad load2(UNom = 400, P = 5000, Q = 100) annotation (
          Placement(visible = true, transformation(origin = {78, 42}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        ...
        Components.myTransformer tra(
          ...
          UNomB=400, R = 0.1) annotation (
          Placement(transformation(extent = {{-40, 20}, {-20, 40}})));
        Components.myPVNode pvNode(UNom = 400, Pstart = -2000) annotation (
          Placement(transformation(extent = {{74, 66}, {94, 86}})));
        Components.myBreaker brk2 annotation (
          Placement(transformation(extent = {{16, 66}, {36, 86}})));
        Modelica.Blocks.Logical.Not n annotation (
          Placement(transformation(extent = {{-6, -6}, {6, 6}}, rotation = 180, origin = {-42, 58})));
        Components.myLine line4(
          UNom=400,             Imax = 50, R = 0.1) annotation (
          Placement(visible = true, transformation(origin = {56, 76}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      equation
        connect(load2.terminal, line2.terminalB) annotation (
          Line(points={{77.96,42.02},{66,42.02},{66,42},{54,42}},
                                              color = {0, 0, 0}));
        connect(line3.terminalB, load3.terminal) annotation (
          Line(points={{54,18},{66,18},{66,18.02},{77.96,18.02}},
                                              color = {0, 0, 0}));
        connect(brk1.terminalB, line2.terminalA) annotation (
          Line(points = {{10, 30}, {22, 30}, {22, 42}, {34, 42}}, color = {0, 0, 0}));
        connect(brk1.terminalB, line3.terminalA) annotation (
          Line(points = {{10, 30}, {22, 30}, {22, 18}, {34, 18}}, color = {0, 0, 0}));
        connect(cmd.y, brk1.BrkOpen) annotation (
          Line(points = {{-15.4, -4}, {0, -4}, {0, 27}}, color = {255, 0, 255}));
        connect(src.terminal, line1.terminalA) annotation (
          Line(points={{-88,30},{-70,30}},      color = {0, 0, 0}));
        connect(line1.terminalB, tra.terminalA) annotation (
          Line(points={{-50,30},{-40,30}},      color = {0, 0, 0}));
        connect(tra.terminalB, brk1.terminalA) annotation (
          Line(points = {{-20, 30}, {-10, 30}}, color = {0, 0, 0}));
        connect(n.u, brk1.BrkOpen) annotation (
          Line(points = {{-34.8, 58}, {-22, 58}, {-22, 62}, {-18, 62}, {-18, 84}, {-124, 84}, {-124, 12}, {-8, 12}, {-8, -4}, {0, -4}, {0, 27}}, color = {255, 0, 255}));
        connect(n.y, brk2.BrkOpen) annotation (
          Line(points = {{-48.6, 58}, {-56, 58}, {-56, 44}, {0, 44}, {0, 60}, {28, 60}, {28, 73}, {26, 73}}, color = {255, 0, 255}));
        connect(brk2.terminalB, line4.terminalA) annotation (
          Line(points = {{36, 76}, {46, 76}}, color = {0, 0, 0}));
        connect(line4.terminalB, pvNode.terminal) annotation (
          Line(points = {{66, 76}, {84, 76}}, color = {0, 0, 0}));
        connect(brk2.terminalA, line3.terminalA) annotation (Line(points={{16,
                76},{18,76},{18,18},{34,18}}, color={0,0,0}));
        connect(brk2.terminalA, line2.terminalA) annotation (Line(points={{16,
                76},{18,76},{18,42},{34,42}}, color={0,0,0}));
        annotation (
          Icon(coordinateSystem(preserveAspectRatio = false)),
          Diagram(coordinateSystem(preserveAspectRatio = false), graphics={  Text(lineColor = {28, 108, 200}, extent = {{-118, -16}, {122, -90}}, fontSize = 12, textString = "brk1 switches off and brk2 switch on at 0.5 s
...
Pstart=-2 kW for the PV node")}),
      ...
      model Islanding3 "Two loads with opposite breakers"
        ...
        Components.myPVNode pv2(UNom = 400, Pstart = -2000) annotation (
        ...
        Components.myPVNode pv3(UNom = 400, Pstart = -2000) annotation (
      ...
      equation
        connect(pv2.terminal, line2.terminalB) annotation (
          Line(points={{86,42},{60,42}},      color = {0, 0, 0}));
        connect(line3.terminalB, load3.terminal) annotation (
          Line(points={{60,18},{72,18},{72,18.02},{79.96,18.02}},
                                              color = {0, 0, 0}));
        connect(brk.terminalB, line2.terminalA) annotation (
        ...
        connect(pv3.terminal, line4.terminalB) annotation (
          Line(points={{86,-2},{60,-2}},      color = {0, 0, 0}));
        connect(line5.terminalB, load5.terminal) annotation (
          Line(points={{60,-26},{72,-26},{72,-25.98},{79.96,-25.98}},
                                                color = {0, 0, 0}));
        ...
        annotation (
          Icon(coordinateSystem(preserveAspectRatio = false)),
          Diagram(coordinateSystem(preserveAspectRatio = false), graphics={  Text(lineColor = {28, 108, 200}, extent = {{36, 78}, {112, 56}}, fontSize = 12, textString = "P=5 kW and Q=0.1 kvar for the load
Pstart=-5 kW and Qstart=0 kvar for the PV node"), Text(lineColor = {28, 108, 200}, extent={{
                    -118,-30},{122,-104}},                                                                              fontSize = 12, textString = "the circuit breaker switches off at 0.5 s
consuming loads are permanently supplied
at the beginning by the source and then by the producing load")}));
      ...
      model DistrictWithBreakers "District with two breakers"
        ...
        Components.myLine line1(UNom=20000,
                                R = 1) annotation (
      ...
      equation
        ...
        connect(load.terminal, line.terminalB) annotation (
          Line(points={{99.96,0.02},{88,0.02},{88,0},{78,0}},
                                             color = {0, 0, 0}));
        connect(load21.terminal, line21.terminalB) annotation (
          Line(points={{99.96,-47.98},{76,-47.98},{76,-48},{52,-48}},
                                                 color = {0, 0, 0}));
        connect(load22.terminal, line211.terminalB) annotation (
          Line(points={{99.96,-79.98},{96,-79.98},{96,-80},{90,-80}},
                                                 color = {0, 0, 0}));
        connect(load12.terminal, line111.terminalB) annotation (
          Line(points={{99.96,80.02},{96,80.02},{96,80},{90,80}},
                                               color = {0, 0, 0}));
        connect(src.terminal, tra.terminalA) annotation (
        ...
        connect(line11.terminalB, load11.terminal) annotation (
          Line(points={{52,48},{74,48},{74,48.02},{99.96,48.02}},
                                               color = {0, 0, 0}));
...
all loads are permanently supplied")}));
    ...
    package ExampleWithFMUs "Small MV-LV network"
      ...
      model MVLVNetwork "Whole network"
        ...
        Components.myLine line2(
          UNom=400,             Imax = 60, R = 2.157870e-02, X = 2.657870e-02) annotation (
      ...
      equation
        ...
        connect(load2.terminal, line3.terminalB) annotation (
          Line(points={{33.96,20.02},{28,20.02},{28,20},{24,20}},
                                              color = {0, 0, 0}));
        connect(tra2.terminalB, line3.terminalA) annotation (
        ...
        annotation (
          Diagram(graphics={  Text(lineColor = {28, 108, 200}, extent={{8,42},{
                    186,2}},                                                                   fontSize = 12, textString = "P=5 kW and Q=0 kvar"), Text(lineColor = {28, 108, 200}, extent = {{-32, -8}, {42, -52}}, fontSize = 12, textString = "current is 7.22 A in each load
and 0.138 A in the source")}, coordinateSystem(initialScale = 0.1)));
      ...
      equation
        ...
        connect(load2.terminal, line3.terminalB) annotation (
          Line(points={{75.96,20.02},{72,20.02},{72,20},{68,20}},
                                              color = {0, 0, 0}));
        connect(bout.i_re, bin.i_re) annotation (
...
and 0.138 A in the source"), Text(lineColor = {28, 108, 200}, extent={{38,46},{
                    236,-8}},                                                                 fontSize = 12, textString = "P=5 kW and Q=0 kvar")}, coordinateSystem(initialScale = 0.1)));
...
the MV part with causal inputs/outputs")}));
      ...
      equation
        ...
        connect(load2.terminal, line3.terminalB) annotation (
          Line(points={{67.96,20.02},{62,20.02},{62,20},{58,20}},
                                              color = {0, 0, 0}));
        connect(bin.terminal, line3.terminalA) annotation (
...
the same model is then cut in 2 parts to get FMUs")}));
    ...
    package MediumNetworks "Medium MV-LV network"
      ...
      model Network "Simple Distribution network"
        ...
        Components.myLine line2(
          UNom=400,             Imax = 140, R = 0.2, X = 0.2) annotation (
      ...
      equation
        ...
        connect(load2.terminal, line3.terminalB) annotation (
          Line(points={{79.96,16.02},{70,16.02},{70,16},{60,16}},
                                              color = {0, 0, 0}));
        connect(tra2.terminalB, line3.terminalA) annotation (
        ...
        connect(line5.terminalB, load4.terminal) annotation (
          Line(points={{98,38},{102,38},{102,38.02},{109.96,38.02}},
                                               color = {0, 0, 0}));
        connect(src.terminal, tra1.terminalA)
          annotation (Line(points={{-88,16},{-72,16},{-72,16}}, color={0,0,0}));
        annotation (
...
36 kVA (Q=0.4*P)"),                                                                                                                                        Text(lineColor = {28, 108, 200}, extent={{
                    -110,42},{-66,28}},                                                                                                                                                                                       textStyle = {TextStyle.Bold}, fontSize = 10, textString = "RTE"), Text(lineColor = {28, 108, 200}, extent = {{-42, -22}, {32, -66}}, fontSize = 12, textString = "low voltage is detected in 
load1 and load4")}, coordinateSystem(initialScale = 0.1)));
      ...
      model DoubleNetwork "Same network with two LV feeders"
        ...
        Components.myLine line8(
          UNom=400,             Imax = 140, R = 0.2, X = 0.2) annotation (
          Placement(visible = true, transformation(origin = {52, -62}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
        Components.myLine line9(
          UNom=400,             Imax = 140, R = 0.2, X = 0.2) annotation (
      ...
      equation
        ...
        connect(load2.terminal, line3.terminalB) annotation (
          Line(points={{79.96,38.02},{70,38.02},{70,38},{60,38}},
                                              color = {0, 0, 0}));
        connect(tra2.terminalB, line3.terminalA) annotation (
        ...
        connect(line5.terminalB, load4.terminal) annotation (
          Line(points={{98,60},{102,60},{102,60.02},{109.96,60.02}},
                                               color = {0, 0, 0}));
        ...
        connect(load6.terminal, line8.terminalB) annotation (
          Line(points={{81.96,-61.98},{72,-61.98},{72,-62},{62,-62}},
                                                color = {0, 0, 0}));
        ...
        connect(line9.terminalB, load8.terminal) annotation (
          Line(points={{100,-40},{104,-40},{104,-39.98},{111.96,-39.98}},
                                                 color = {0, 0, 0}));
...
36 kVA (Q=0.4*P)")}, coordinateSystem(initialScale = 0.1)),
    ...
    model FourVariableLoadsOneLVFeeder "Realistic case with three LV feeders and four variable loads"
      ...
      Components.myLine LVln2(
        UNom=400,             R = 0.2, X = 0.2, Imax = 45) annotation (
        Placement(visible = true, transformation(origin={18,0},   extent = {{-10, -10}, {10, 10}}, rotation = 0)));
      Components.myVariableTransformer HVMV(
        UNomA=63000,
        UNomB=20000,
        SNom=100000, R = 1e-6) annotation (
        Placement(transformation(extent = {{-102, -10}, {-82, 10}})));
      Components.myTransformer MVLV(
        UNomA=20000,
        UNomB=400,
        SNom=250, R = 1e-9) annotation (
        Placement(transformation(extent={{-42,-10},{-22,10}})));
      Components.myLine MVline(
        UNom=20000,            R = 0.9, X = 0.3) annotation (
        Placement(transformation(extent = {{-72, -10}, {-52, 10}})));
      Components.myLine LVln1(
        UNom=400,             R = 0.2, X = 0.2, Imax = 45) annotation (
        Placement(transformation(extent = {{-6, 30}, {14, 50}})));
      Components.myLine LVln11(
        UNom=400,              R = 0.2, X = 0.2, Imax = 45) annotation (
        Placement(transformation(extent = {{22, 30}, {42, 50}})));
      Components.myLine LVln3(
        UNom=400,             R = 0.2, X = 0.2, Imax = 45) annotation (
    ...
    equation
      ...
      connect(LVln11.terminalB, varLoad2.terminal) annotation (
        Line(points={{42,40},{48,40},{48,40.02},{51.96,40.02}},
                                            color = {0, 0, 0}));
      connect(LVln2.terminalB, varLoad3.terminal) annotation (
        Line(points={{28,0},{32,0},{32,0.02},{51.96,0.02}},
                                          color = {0, 0, 0}));
      connect(LVln3.terminalB, varLoad4.terminal) annotation (
        Line(points={{28,-40},{32,-40},{32,-39.98},{49.96,-39.98}},
                                              color = {0, 0, 0}));
      ...
      connect(LVln11.terminalB, varLoad1.terminal) annotation (
        Line(points={{42,40},{42,80.02},{53.96,80.02}},
                                                      color = {0, 0, 0}));
      annotation (
        experiment(StopTime = 31532400, __Dymola_Algorithm = "Dassl"),
...
simulation is done for 1-year duration")}),
    ...
    model LargerNetwork "This model has been partly generated by a Python tool"
      ...
      Components.mySource src(UNom=225000)    annotation (Placement(transformation(extent={{-270,94},
                {-250,114}})));
      Components.myTransformer hvmv(UNomA=225000,UNomB=20000,R=7.250000e-01,X=2*3.1456*50*1.151031e-01)    annotation (Placement(transformation(extent={{-210,94},
                {-190,114}})));
      Components.myTransformer mvlv11(UNomA=20000,UNomB=400,R=6.679134e+00,X=2*3.1456*50*6.764085e-02)    annotation (Placement(transformation(extent={{26,46},
                {46,66}})));
      Components.myTransformer mvlv24(UNomA=20000,UNomB=400,R=1.220206e+01,X=2*3.1456*50*1.069521e-01)    annotation (Placement(transformation(extent={{-10,-10},
                {10,10}},
            rotation=0,
            origin={36,-58})));

      Components.myLine ln1(UNom=20000, l=500, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-80,52},
                {-60,72}})));
      Components.myLine ln3(UNom=20000, l=1000, R=1.130000e-04, X=3.900000e-04, B=1.570000e-09)    annotation (Placement(transformation(extent={{-242,94},
                {-222,114}})));
      Components.myLine ln677(UNom=20000, l=4.314298e+03, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-170,30},
                {-150,50}})));
      Components.myLine ln714(UNom=20000, l=2.163435e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-142,
                -30},{-122,-10}})));
      Components.myLine ln715(UNom=20000, l=1.029283e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-132,26},
                {-112,46}})));
      Components.myLine ln716(UNom=20000, l=2.065060e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-100,
                -76},{-80,-56}})));
      Components.myLine ln717(UNom=20000, l=1.040661e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-102,
                -52},{-82,-32}})));
      Components.myLine ln718(UNom=20000, l=6.536973e+01, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-100,2},
                {-80,22}})));
      Components.myLine ln719(UNom=20000, l=7.358724e+01, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-70,
                -102},{-50,-82}})));
      Components.myLine ln676(UNom=20000, l=4.122333e+03, R=1.354310e-04, X=1.854310e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{-52,94},
                {-32,114}})));
      Components.myLine ln702(UNom=20000, l=2.030604e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-32,-36},
                {-12,-16}})));
      Components.myLine ln703(UNom=20000, l=4.903295e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-28,70},
                {-8,90}})));
      Components.myLine ln712(UNom=20000, l=1.313188e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-4,-68},
                {16,-48}})));
      Components.myLine ln713(UNom=20000, l=8.874849e+01, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{-4,46},
                {16,66}})));
      Components.myLine ln720(UNom=20000, l=1.787394e+02, R=2.000000e-04, X=1.000000e-04, B=5.620000e-08)    annotation (Placement(transformation(extent={{20,-94},
                {40,-74}})));
      Components.myLine ln769(UNom=400, l=1.372877e+02, R=1.354310e-04, X=1.854310e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{58,-16},
                {78,4}})));
      Components.myLine ln770(UNom=400, l=1.535058e+02, R=1.354310e-04, X=1.854310e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{60,24},
                {80,44}})));
      Components.myLine ln771(UNom=400, l=2.126950e+02, R=1.354310e-04, X=1.854310e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{60,46},
                {80,66}})));
      Components.myLine ln772(UNom=400, l=3.493784e+01, R=3.443400e-04, X=3.943400e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{82,-42},
                {102,-22}})));
      Components.myLine ln773(UNom=400, l=1.811677e+01, R=3.443400e-04, X=3.943400e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{116,-66},
                {136,-46}})));
      Components.myLine ln774(UNom=400, l=3.051450e+01, R=3.443400e-04, X=3.943400e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{86,4},{
                106,24}})));
      Components.myLine ln775(UNom=400, l=5.738786e+01, R=1.354310e-04, X=1.854310e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{54,-68},
                {74,-48}})));
      Components.myLine ln776(UNom=400, l=1.100241e+02, R=2.157870e-04, X=2.657870e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{82,-90},
                {102,-70}})));
      Components.myLine ln777(UNom=400, l=1.300031e+02, R=3.443400e-04, X=3.943400e-04, B=5.000000e-08)    annotation (Placement(transformation(extent={{82,-110},
                {102,-90}})));
      ...
      Components.myVariableLoad b61280(UNom=20000)    annotation (Placement(transformation(extent={{-140,56},
                {-120,76}})));
      ...
      Components.myVariableLoad b11458(UNom=20000)    annotation (Placement(transformation(extent={{-106,
                -30},{-86,-10}})));
      ...
      Components.myVariableLoad b61256(UNom=20000)    annotation (Placement(transformation(extent={{-110,26},
                {-90,46}})));
      ...
      Components.myVariableLoad b11480(UNom=20000)    annotation (Placement(transformation(extent={{-82,2},
                {-62,22}})));
      ...
      Components.myVariableLoad b61232(UNom=20000)    annotation (Placement(transformation(extent={{-76,-76},
                {-56,-56}})));
      ...
      Components.myVariableLoad b61121(UNom=20000)    annotation (Placement(transformation(extent={{-76,-52},
                {-56,-32}})));
      ...
      Components.myVariableLoad b87694(UNom=20000)    annotation (Placement(transformation(extent={{-50,
                -102},{-30,-82}})));
      ...
      Components.myVariableLoad b11465(UNom=20000)    annotation (Placement(transformation(extent={{-30,94},{-10,114}})));
      ...
      Components.myVariableLoad b61260(UNom=20000)    annotation (Placement(transformation(extent={{-4,70},{16,90}})));
      ...
      Components.myVariableLoad b61254(UNom=400)    annotation (Placement(transformation(extent={{82,-16},{102,4}})));
      ...
      Components.myVariableLoad b11407(UNom=400)    annotation (Placement(transformation(extent={{108,-42},{128,-22}})));
      ...
      Components.myVariableLoad b11403(UNom=400)    annotation (Placement(transformation(extent={{136,-66},{156,-46}})));
      ...
      Components.myVariableLoad b61227(UNom=20000)    annotation (Placement(transformation(extent={{-10,-36},{10,-16}})));
      ...
      Components.myVariableLoad b61089(UNom=20000)    annotation (Placement(transformation(extent={{38,-94},{58,-74}})));
      ...
      Components.myVariableLoad b61274(UNom=400)    annotation (Placement(transformation(extent={{84,24},{104,44}})));
      ...
      Components.myVariableLoad b61273(UNom=400)    annotation (Placement(transformation(extent={{100,4},{120,24}})));
      ...
      Components.myVariableLoad b61275(UNom=400)    annotation (Placement(transformation(extent={{84,46},{104,66}})));
      ...
      Components.myVariableLoad b56131(UNom=400)    annotation (Placement(transformation(extent={{76,-68},{96,-48}})));
      ...
      Components.myVariableLoad b87656(UNom=400)    annotation (Placement(transformation(extent={{100,-90},{120,-70}})));
      ...
      Components.myVariableLoad b61106(UNom=400)    annotation (Placement(transformation(extent={{100,-110},{120,-90}})));
    ...
    equation
      ...
      connect(src.terminal, ln3.terminalA)    annotation (Line(points={{-260,104},{-242,
              104}},                                   color={0,0,0}));
      connect(ln3.terminalB, hvmv.terminalA)    annotation (Line(points={{-222,104},
              {-210,104}},                                                                     color={0,0,0}));
      connect(hvmv.terminalB, ln676.terminalA) annotation (Line(points={{-190,104},{
              -52,104}},                color={0,0,0}));
      connect(ln676.terminalB, b11465.terminal) annotation (Line(points={{-32,104},{
              -28,104},{-28,104.02},{-20.04,104.02}}, color={0,0,0}));
      connect(ln703.terminalB, b61260.terminal) annotation (Line(points={{-8,80},{-2,
              80},{-2,80.02},{5.96,80.02}}, color={0,0,0}));
      connect(ln676.terminalB, ln703.terminalA)    annotation (Line(points={{-32,104},{-32,80},{-28,80}},   color={0,0,0}));
      connect(ln713.terminalB, mvlv11.terminalA)    annotation (Line(points={{16,56},{26,56}},   color={0,0,0}));
      connect(ln771.terminalB, b61275.terminal) annotation (Line(points={{80,56},{86,
              56},{86,56.02},{93.96,56.02}}, color={0,0,0}));
      connect(ln770.terminalB, b61274.terminal) annotation (Line(points={{80,34},{93.96,
              34},{93.96,34.02}}, color={0,0,0}));
      connect(ln774.terminalB, b61273.terminal) annotation (Line(points={{106,14},{108,
              14},{108,14.02},{109.96,14.02}}, color={0,0,0}));
      connect(ln769.terminalB, b61254.terminal) annotation (Line(points={{78,-6},{84,
              -6},{84,-5.98},{91.96,-5.98}}, color={0,0,0}));
      connect(mvlv11.terminalB, ln769.terminalA) annotation (Line(points={{46,56},{46,
              -8},{58,-8},{58,-6}},          color={0,0,0}));
      connect(mvlv11.terminalB, ln771.terminalA)    annotation (Line(points={{46,56},{60,56}},   color={0,0,0}));
      connect(mvlv11.terminalB, ln770.terminalA) annotation (Line(points={{46,56},{52,
              56},{52,34},{60,34}},       color={0,0,0}));
      connect(ln769.terminalB, ln772.terminalA)    annotation (Line(points={{78,-6},{78,-32},{82,-32}},
                                                           color={0,0,0}));
      connect(ln772.terminalB, b11407.terminal) annotation (Line(points={{102,-32},{
              112,-32},{112,-31.98},{117.96,-31.98}}, color={0,0,0}));
      connect(ln772.terminalB, ln773.terminalA)    annotation (Line(points={{102,-32},{102,-56},{116,-56}},
                                                     color={0,0,0}));
      connect(ln773.terminalB, b11403.terminal) annotation (Line(points={{136,-56},{
              140,-56},{140,-55.98},{145.96,-55.98}}, color={0,0,0}));
      connect(ln676.terminalB, ln702.terminalA)    annotation (Line(points={{-32,104},{-32,-26}},  color={0,0,0}));
      connect(ln703.terminalB, ln713.terminalA)    annotation (Line(points={{-8,80},{-8,56},{-4,56}},    color={0,0,0}));
      connect(ln702.terminalB, b61227.terminal) annotation (Line(points={{-12,-26},{
              -4,-26},{-4,-25.98},{-0.04,-25.98}}, color={0,0,0}));
      connect(ln702.terminalB, ln712.terminalA) annotation (Line(points={{-12,-26},{
              -8,-26},{-8,-58},{-4,-58}},    color={0,0,0}));
      connect(ln712.terminalB, mvlv24.terminalA)    annotation (Line(points={{16,-58},{26,-58}},   color={0,0,0}));
      connect(mvlv24.terminalB, ln775.terminalA)    annotation (Line(points={{46,-58},{54,-58}},   color={0,0,0}));
      connect(ln775.terminalB, b56131.terminal) annotation (Line(points={{74,-58},{76,
              -58},{76,-57.98},{85.96,-57.98}}, color={0,0,0}));
      connect(ln775.terminalB, ln776.terminalA)    annotation (Line(points={{74,-58},{74,-80},{82,-80}},
                                                     color={0,0,0}));
      connect(ln776.terminalB, b87656.terminal) annotation (Line(points={{102,-80},{
              102,-79.98},{109.96,-79.98}}, color={0,0,0}));
      connect(ln775.terminalB, ln777.terminalA)    annotation (Line(points={{74,-58},{74,-100},{82,-100}},
                                                      color={0,0,0}));
      connect(ln777.terminalB, b61106.terminal) annotation (Line(points={{102,-100},
              {104,-100},{104,-99.98},{109.96,-99.98}}, color={0,0,0}));
      connect(ln712.terminalB, ln720.terminalA)    annotation (Line(points={{16,-58},{16,-84},{20,-84}},    color={0,0,0}));
      connect(ln720.terminalB, b61089.terminal) annotation (Line(points={{40,-84},{44,
              -84},{44,-83.98},{47.96,-83.98}}, color={0,0,0}));
      connect(ln677.terminalB, b61280.terminal) annotation (Line(points={{-150,40},{
              -138,40},{-138,66.02},{-130.04,66.02}}, color={0,0,0}));
      connect(ln677.terminalB, ln715.terminalA) annotation (Line(points={{-150,40},{
              -140,40},{-140,36},{-132,36}},
                                           color={0,0,0}));
      connect(ln715.terminalB, b61256.terminal) annotation (Line(points={{-112,36},{
              -106,36},{-106,36.02},{-100.04,36.02}}, color={0,0,0}));
      connect(ln718.terminalB, b11480.terminal) annotation (Line(points={{-80,12},{-76,
              12},{-76,12.02},{-72.04,12.02}}, color={0,0,0}));
      connect(ln677.terminalB, ln714.terminalA)    annotation (Line(points={{-150,40},
              {-150,-20},{-142,-20}},                                                                        color={0,0,0}));
      connect(ln717.terminalB, b61121.terminal) annotation (Line(points={{-82,-42},{
              -74,-42},{-74,-41.98},{-66.04,-41.98}},  color={0,0,0}));
      connect(ln716.terminalB, ln719.terminalA) annotation (Line(points={{-80,-66},{
              -80,-92},{-70,-92}},      color={0,0,0}));
      connect(ln719.terminalB, b87694.terminal) annotation (Line(points={{-50,-92},{
              -50,-91.98},{-40.04,-91.98}}, color={0,0,0}));
      connect(ln716.terminalB, b61232.terminal) annotation (Line(points={{-80,-66},{
              -72,-66},{-72,-65.98},{-66.04,-65.98}}, color={0,0,0}));
      connect(ln714.terminalB, ln716.terminalA) annotation (Line(points={{-122,-20},
              {-122,-66},{-100,-66}}, color={0,0,0}));
      connect(ln714.terminalB, b11458.terminal) annotation (Line(points={{-122,-20},
              {-122,-19.98},{-96.04,-19.98}},  color={0,0,0}));
      connect(ln714.terminalB, ln717.terminalA) annotation (Line(points={{-122,-20},
              {-122,-42},{-102,-42}}, color={0,0,0}));
      connect(ln770.terminalB, ln774.terminalA)    annotation (Line(points={{80,34},{80,14},{86,14}}, color={0,0,0}));
      connect(ln715.terminalB, ln1.terminalA) annotation (Line(points={{-112,36},{-106,
              36},{-106,62},{-80,62}}, color={0,0,0}));
      connect(ln715.terminalB, ln718.terminalA) annotation (Line(points={{-112,36},{
              -106,36},{-106,12},{-100,12}}, color={0,0,0}));
      connect(hvmv.terminalB, ln677.terminalA) annotation (Line(points={{-190,104},{
              -178,104},{-178,40},{-170,40}}, color={0,0,0}));
      connect(ln712.terminalB, ln1.terminalB) annotation (Line(points={{16,-58},{16,
              -72},{-44,-72},{-44,62},{-60,62}}, color={0,0,0}));
      annotation (experiment(StopTime=0, __Dymola_Algorithm="Dassl"));
    end LargerNetwork;
    ...
    Real MaxP = 40000;
    Real MaxQ = 0;
    Real Sreduced = 1000 * load.S;
    Real Srequired = sqrt(load.PInput ^ 2 + load.QInput ^ 2);
    Real C;
...
impedance mode at time 200 s
when voltage is too low
The apparent power of the load is reduced
and can be compared to the required one")}),        experiment(StopTime=400,
...
<p>For further technical information, see the <a href=\"modelica://PowerSysPro.Information\">Information</a>.</p>
    ...
    Diagram(graphics={  Text(lineColor = {28, 108, 200}, extent = {{-176, 34}, {178, -22}}, fontSize = 14, textStyle = {TextStyle.Bold}, textString = "Open library developed at EDF Lab. Paris-Saclay")}));
end PowerSysPro;
